...
/*   Updated: 2022/02/28 23:00:54 by pat              ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
	...
	while (1)
	{
		if (vector.p1.y + ref.y >= 0 && vector.p1.y + ref.y < w->height
			...
			mlx_pixel_put(w->mlx_ptr, w->win_ptr, ft_round(vector.p1.x + ref.x), 
				ft_round(vector.p1.y + ref.y), ft_color_bresenham(vector.p1.z_originel, 
				vector.p2.z_originel, 1 - ((double)pythagore(vector.p1,vector.p2) / (double)pythagore(p, vector.p2)), w));
